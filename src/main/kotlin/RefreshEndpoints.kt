...
//            val meetupGroups = (MeetupApi.findGroups("gdg") + MeetupApi.findGroups("Google Cloud Developer Community")).filter { it.gdgName.toSlug().startsWith("gdg-") }
...
//        }
        ...
        get("firebase/from/meetup") {
            val urlname = call.parameters["name"]
            if (urlname != null) {
                val group = MeetupApi.getGroupDetail(urlname)
                if (group !=  null) {
                    val groupDao = group.toDao()
                    FireDB.addGroup(groupDao)
                    FireDB.addMeetupUrlnames(groupDao.slug, group.urlname)
                    val events = MeetupApi.getEvents(group.urlname).map { it.toDao() }
                    FireDB.addEvents(events)
                    call.respondText(groupDao.toResponse(events.toEventResponseList()).toJsonPretty(GroupResponse.serializer()))
                } else {
                    call.respondText("$urlname does not exist")
                }
            } else {
                call.respondText("Missing 'name' parameter")
            }
        }
